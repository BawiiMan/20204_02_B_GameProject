...
public class SkillTreeUI : MonoBehaviour
{
    ...
    void InitalizeSkillTree()
    {
        ...
        skillTree.AddNode(new SkillNode("Firebal2", "Fireball2",
            new Skill<ISkillTarget, DamageEffect>("Fireball2", new DamageEffect(30)),
            new Vector2(0, 1), "Fireball2", 2, new List<string> { "Fireball1"}));

        skillTree.AddNode(new SkillNode("Fireball3", "Fireball3",
            new Skill<ISkillTarget, DamageEffect>("Fireball3", new DamageEffect(40)),
            new Vector2(0, 2), "Fireball3", 3, new List<string> { "Fireball2" }));

        skillTree.AddNode(new SkillNode("Fireball4", "Fireball4",
            new Skill<ISkillTarget, DamageEffect>("Fireball4", new DamageEffect(50)),
            new Vector2(0, 3), "Fireball4", 4, new List<string> { "Fireball3" }));

        skillTree.AddNode(new SkillNode("FireBolt1", "FireBolt1",
            new Skill<ISkillTarget, DamageEffect>("FireBolt11", new DamageEffect(90)),
            new Vector2(1, 2), "FireBolt1", 1, new List<string> { "Fireball2" }));
        skillTree.AddNode(new SkillNode("FireBolt2", "FireBolt2",
            new Skill<ISkillTarget, DamageEffect>("FireBolt11", new DamageEffect(140)),
            new Vector2(2, 2), "FireBolt1", 2, new List<string> { "FireBolt1" }));
        skillTree.AddNode(new SkillNode("FireBolt3", "FireBolt3",
            new Skill<ISkillTarget, DamageEffect>("FireBolt11", new DamageEffect(240)),
            new Vector2(3, 2), "FireBolt1", 3, new List<string> { "FireBolt2" }));
    }
    ...
    private bool CanUnlockSkill(SkillNode node)
    {
        ...
        {
            if(!skillTree.IsSkillUnlock(requiredSkilld))
            {
                return false;
            }
        }
